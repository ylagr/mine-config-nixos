{ config, home-manager, username, homedir,  pkgs, ... }:
let
  emacs-app= "emacs-igc-pgtk";
in
{
  # home.username = "ylagr";
  ...
  programs.emacs = {
    enable = true;
    package = pkgs.${emacs-app}.pkgs.withPackages (epkgs: with epkgs; [ 
  ...
  programs = {
    ...
    bash = {
      ...
      bashrcExtra = ''
          ...
          if [ -e /home/ylagr/.nix-profile/etc/profile.d/nix.sh ]; then
              . /home/ylagr/.nix-profile/etc/profile.d/nix.sh;
